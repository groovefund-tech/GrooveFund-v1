...
export default function Dashboard() {
  ...
  const checkProfile = async () => {
    try {
      ...
      if (error || !data)
        setProfileError("We're setting up your Groove account. Please refresh the page.")
      else
  ...
  const backfillPayment = async () => {
    ...
    try {
      const res = await fetch('/api/admin/backfill-payment', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ target_user_id: member.user_id, amount: backfillAmount })
      })
  ...
  const handleTopUp = async (amount: number) => {
    ...
    try {
      const res = await fetch('/api/create-checkout', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ amount: topUpAmount, user_id: member.user_id })
      })
      const data = await res.json()
      if (!res.ok) {
  ...
  const loadDashboard = useCallback(async () => {
    ...
    try {
      ...
      const avail = Math.max(0, tSlots - slotsUsed)
  ...
  const applyAdminPayment = async () => {
    ...
    try {
      await fetch('/api/admin/apply-payment', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ target_user_id: member.user_id, amount: 500 })
      })
      await loadDashboard()
      setErrorMessage(null)
    } catch (err) {
          ...
          <main style={{ maxWidth: '1440px', margin: '0 auto', padding: '32px 24px', overflowX: 'hidden' }}>
            ...
            <section style={{ marginBottom: '48px' }}>
              <h1 style={{ fontSize: '48px', fontWeight: 800, color: '#1F2937', margin: '0 0 8px 0' }}>
                Hey {member?.display_name?.split(' ')[0] || 'there'} 👋
              </h1>
            ...
            {!dismissedLeaderboardTip && (
              <div style={{ padding: '12px 16px', background: '#FFF5ED', borderRadius: '12px', border: '1px solid #FFE4CC', marginBottom: '16px', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '12px' }}>
                <div style={{ fontSize: '12px', color: '#6B7280', lineHeight: '1.5', flex: 1 }}>
                  <strong style={{ color: '#FF751F', display: 'block', marginBottom: '4px' }}>
                    📊 Welcome To The GrooveFund Community!
                  </strong>
            ...
            <section style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '16px', marginBottom: '48px' }}>
              ...
              {/* Info Banner */}
              <div style={{ background: '#F0F9FF', border: '1px solid #BFDBFE', borderRadius: 12, padding: 16, marginBottom: 24 }}>
            ...
            {userStreak.currentMonth > 0 && (
              ...
              >
                <div style={{ fontSize: '48px', marginBottom: '8px' }}>{userStreak.tier?.emoji}</div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '24px', marginBottom: '48px' }}>
              {/* NEXT GROOVES */}
              <section>
                <h2 style={{ fontSize: '28px', fontWeight: 800, color: '#1F2937', margin: '0 0 24px 0', paddingBottom: '16px', borderBottom: '3px solid rgba(255, 117, 31, 0.15)', letterSpacing: '-0.5px' }}>
                  🥳 Your Locked Grooves
                </h2>
                {nextGrooves.length === 0 ? (
                  <div className="empty-state">
                    <div className="empty-state-title">No Grooves locked yet</div>
                    <div className="empty-state-text">Lock your spot for a Groove below</div>
                  </div>
                ) : (
                    ...
                    {nextGrooves.map((e: any) => (
                      ...
                      >
                        ...
                        <div style={{ padding: '16px' }}>
                          <h3 style={{ fontSize: '16px', fontWeight: 700, color: '#1F2937', margin: '0 0 12px 0' }}>
                            {e.name}
                          </h3>
                          ...
                          >
                            <span style={{ fontSize: '11px', color: '#FF751F', fontWeight: 700 }}>
                              {e.slot_cost && e.slot_cost > 1
                                ? `uses ${e.slot_cost} spots`
                                : 'uses 1 spot'}
                            </span>
              ...
              <section>
                ...
                {suggested.length === 0 ? (
                  <div className="empty-state">
                    <div className="empty-state-title">No Grooves available yet.</div>
                    <div className="empty-state-text">
                      Check back soon for new community Grooves
                    </div>
                  </div>
                ) : (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                      ...
                      return (
                        ...
                        >
                          ...
                          <div style={{ padding: '16px' }}>
                            ...
                            {/* ADD COUNTDOWN HERE */}
                            {(() => {
                              const daysUntil = event.start_at
                                ? Math.max(
                                    0,
                                    Math.ceil(
                            ...
                            >
                              <span style={{ fontSize: '11px', color: '#FF751F', fontWeight: 700 }}>
                                {event.slot_cost && event.slot_cost > 1
                                  ? `uses ${event.slot_cost} spots`
                                  : 'uses 1 spot'}
                              </span>
            ...
            >
              ...
              >
                <h3 style={{ fontSize: '14px', fontWeight: 700, color: '#1F2937', margin: '0 0 12px 0' }}>
                  ⚡ Quick Actions
                </h3>
                <button
                  onClick={() => setShowTopUp(true)}
          ...
          {showTopUp && (
            <div className="modal-backdrop">
              <div className="modal-content">
                ...
                >
                  <div>
                    New Balance: <strong>{confirmedPoints + topUpAmount} pts</strong>
                  </div>
                  <div>
                    New Spots: <strong>{slotsAfter}</strong>
                  </div>
                </div>
          ...
          {adminNotifications.filter((n: any) => !n.read).length > 0 && (
            ...
            >
              <div style={{ fontSize: '12px', fontWeight: 700, color: '#DC2626', marginBottom: '8px' }}>
                ⚠️{' '}
                {adminNotifications.filter((n: any) => !n.read).length} Action
                {adminNotifications.filter((n: any) => !n.read).length > 1
                  ? 's'
                  : ''}{' '}
                Required
              </div>
