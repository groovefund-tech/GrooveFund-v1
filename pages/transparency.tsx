      ...
      <main style={{ maxWidth: '1200px', margin: '0 auto', padding: '48px 20px' }}>
        ...
        <section style={{ marginBottom: '64px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '24px' }}>
            {[
              { icon: '👥', title: 'Total Members', value: loading ? '...' : data.totalMembers.toLocaleString(), subtitle: 'Groovers saving together' },
              { icon: '💰', title: 'Total Contributed', value: loading ? '...' : `R${data.totalContributed.toLocaleString()}`, subtitle: 'Combined savings pool' },
              { icon: '🏦', title: 'Available Pool', value: loading ? '...' : `R${data.poolSize.toLocaleString()}`, subtitle: 'After 12% admin fee' },
              { icon: '🎫', title: 'Tickets Issued', value: loading ? '...' : data.ticketsIssued.toLocaleString(), subtitle: 'Real concert tickets' },
              { icon: '🔥', title: 'Active Members', value: loading ? '...' : data.activeMembers.toLocaleString(), subtitle: 'Last 40 days' },
              { icon: '💸', title: 'Admin Fee Used', value: loading ? '...' : `R${data.adminFeeTotal.toLocaleString()}`, subtitle: 'Operations & support' },
            ].map((metric, i) => (
        ...
        <section style={{ marginBottom: '64px', background: 'white', padding: '48px', borderRadius: '20px', border: '1px solid #E5E7EB' }}>
          <h2 style={{ fontSize: '28px', fontWeight: 700, color: '#1F2937', marginBottom: '32px' }}>💰 How Your Money Works</h2>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '32px' }}>
            {[
              { step: '1️⃣', title: 'You Contribute', desc: 'You send R500/month via EFT or YOCO. It\'s secure, POPIA-compliant, and encrypted.', example: 'Example: 100 members × R500 = R50,000 total' },
              { step: '2️⃣', title: 'Pool Grows', desc: 'Your money goes into a community pool with thousands of other Groovers. 88% stays in the pool.', example: 'Pool: R50,000 × 0.88 = R44,000 available' },
              { step: '3️⃣', title: 'Admin Fee (12%)', desc: 'We take 12% to cover payments processing, customer support, event partnerships, and platform maintenance.', example: 'Admin fee: R50,000 × 0.12 = R6,000' },
              { step: '4️⃣', title: 'Tickets Purchased', desc: 'The available pool buys real concert and festival tickets. Top 40% members get priority access.', example: 'R44,000 buys tickets to major events' },
              { step: '5️⃣', title: 'You Win Tickets', desc: 'If you\'re in the top 40% by points, you get access to purchase tickets at member prices.', example: '500+ points = qualified for tickets' },
              { step: '6️⃣', title: 'Points Roll Over', desc: 'Didn\'t win this month? Your points stay. No loss. You compound them for future events.', example: 'Month 1: 500pts, Month 2: 1000pts' },
            ].map((item, i) => (
        ...
        {/* Fee Breakdown */}
        <section style={{ marginBottom: '64px' }}>
          <h2 style={{ fontSize: '28px', fontWeight: 700, color: '#1F2937', marginBottom: '32px', textAlign: 'center' }}>💸 Where Your 12% Admin Fee Goes</h2>
          <div style={{ background: 'white', padding: '40px', borderRadius: '16px', border: '1px solid #E5E7EB' }}>
            ...
            <div style={{ marginTop: '32px', padding: '24px', background: '#F0F9FF', border: '1px solid #BFDBFE', borderRadius: '12px', textAlign: 'center' }}>
              <p style={{ fontSize: '14px', color: '#1E40AF', fontWeight: 600, margin: 0 }}>
                ✅ All fees are disclosed upfront. No hidden charges. No surprises.
              </p>
            </div>
          </div>
        </section>
        ...
        {/* Rules Summary */}
        <section style={{ marginBottom: '64px' }}>
          <h2 style={{ fontSize: '28px', fontWeight: 700, color: '#1F2937', marginBottom: '32px', textAlign: 'center' }}>📋 Key Rules Everyone Should Know</h2>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '24px' }}>
            {[
              { icon: '💰', title: 'Contribution', rules: ['R500/month minimum', 'No signup fees', 'Can increase anytime', 'First contribution gets 500 points instantly'] },
              { icon: '🎫', title: 'Winning Tickets', rules: ['Top 40% (500+ pts) get priority', 'More points = better tickets', 'Points roll over monthly', 'No points lost if you don\'t win'] },
              { icon: '🔥', title: 'Streaks & Inactive', rules: ['40+ days inactive = lose 15pts/day', 'One R500 payment resets timer', 'Build streaks for consistency', 'Streaks unlock member rewards'] },
              { icon: '🛡️', title: 'Legal & Safe', rules: ['Operates as traditional stokvel', 'POPIA-compliant payments', 'Full audit trails available', 'Fully transparent & auditable'] },
              { icon: '🎵', title: 'Tickets', rules: ['Real tickets to real events', 'Tickets cost 495 points per slot', '1 slot = R700-R1000 events', '2-3 slots = R1200-R2000 events'] },
              { icon: '💡', title: 'Community', rules: ['5000+ active Groovers', 'Connect with members attending', 'Referral rewards (50 pts)', 'VIP GrooveFund member events'] },
            ].map((section, i) => (
                ...
                <ul style={{ listStyle: 'none', padding: 0, margin: 0 }}>
                  {section.rules.map((rule, j) => (
                    <li key={j} style={{ fontSize: '13px', color: '#6B7280', marginBottom: '10px', display: 'flex', alignItems: 'flex-start', gap: '8px' }}>
                      <span style={{ color: '#FF751F', fontWeight: 700, marginTop: '2px' }}>✓</span>
                      {rule}
                    </li>
                  ))}
                </ul>
        ...
        <section style={{ marginBottom: '64px', background: 'linear-gradient(135deg, #F0F9FF 0%, #E0F2FF 100%)', padding: '48px', borderRadius: '20px', border: '2px solid #BFDBFE' }}>
          <h2 style={{ fontSize: '28px', fontWeight: 700, color: '#1E40AF', marginBottom: '32px', textAlign: 'center' }}>🚨 Why GrooveFund is Reliable</h2>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '24px' }}>
            {[
              { concern: 'Where does my money go?', answer: 'Into a transparent community pool. You can see the exact pool size on your dashboard, updated hourly. Money is used to buy real tickets from real venues.' },
              { concern: 'Will I actually win tickets?', answer: 'Yes. Top 40% by points get tickets. We show tickets bought (this page), members who won, and event details. If you qualify, you get tickets.' },
              { concern: 'What if the company disappears?', answer: 'Pool is managed transparently with audit trails. All payments are documented. If GrooveFund closes, the pool is distributed fairly to members.' },
              { concern: 'Is 12% fee too high?', answer: 'Industry standard for payment processing + support is 12-15%. We\'re transparent about exactly where it goes (see breakdown above).' },
              { concern: 'How do I know it\'s not rigged?', answer: 'Top 40% (500+ points) always get priority. Algorithm is clear: more contributions = more points = better access. No favoritism.' },
              { concern: 'Is my data safe?', answer: 'Yes. All data encrypted, POPIA-compliant, processed through trusted payment providers (YOCO & EFT). We don\'t sell your data.' },
            ].map((item, i) => (
              <div key={i} style={{ background: 'white', padding: '24px', borderRadius: '12px', border: '1px solid #BFDBFE' }}>
                <h3 style={{ fontSize: '15px', fontWeight: 700, color: '#1E40AF', marginBottom: '12px' }}>❓ {item.concern}</h3>
        ...
        <section style={{ background: 'linear-gradient(135deg, #FF751F 0%, #FF8C42 100%)', padding: '48px', borderRadius: '20px', textAlign: 'center', marginBottom: '64px' }}>
          ...
          <button onClick={() => router.push('/signup')} style={{ padding: '14px 40px', background: 'white', color: '#FF751F', border: 'none', borderRadius: '12px', fontSize: '16px', fontWeight: 700, cursor: 'pointer', fontFamily: 'Poppins', transition: 'all 0.3s' }} onMouseEnter={(e) => { e.currentTarget.style.transform = 'scale(1.05)'; e.currentTarget.style.boxShadow = '0 8px 20px rgba(0,0,0,0.2)' }} onMouseLeave={(e) => { e.currentTarget.style.transform = 'scale(1)'; e.currentTarget.style.boxShadow = 'none' }} >
            Start Saving Now 🎵
          </button>
        </section>
      </main>
